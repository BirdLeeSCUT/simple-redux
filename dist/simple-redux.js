...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.createLoggableActions = exports.createPartStore = exports.createReducer = undefined;
	 ...
	 * 
	 */
	var createPartStore = function createPartStore(namespace) {

		var getType = function getType(param) {
			var _t = void 0;
			return ((_t = typeof param === 'undefined' ? 'undefined' : _typeof(param)) == "object" ? Object.prototype.toString.call(param).slice(8, -1) : _t).toLowerCase();
		};

		//供生成 set 操作
		var setRaw = function setRaw() {
			return function (index, newValue) {
				var arrIndex = void 0;
				if (getType(index) == 'string') arrIndex = index.split('/');else if (getType(index) == 'array') arrIndex = index;else throw new Error('Index should be string or array');
		...
		var mergeRaw = function mergeRaw(deep) {
			return function (arg1, arg2) {
				...
				if (getType(arg1) == 'object') {
					...
					var arrIndex = void 0;
					if (getType(arg1) == 'string') arrIndex = arg1.split('.');else if (getType(arg1) == 'array') arrIndex = arg1;else throw new Error('Index should be string or array');
	   ...
	   */
			get state() {
				var selectorArr = namespace.split('/');

				return selectorArr.reduce(function (preValue, thisVale) {
	   ...
	   */
			merge: function merge(diff) {
				return mergeRaw(false)(diff);
			},

			/**
	   * mergeIn 操作，集成了 immutable::Map 的
	   */
			mergeIn: function mergeIn(arg1, arg2) {
				return mergeRaw(false)(arg1, arg2);
			},
	 ...
	 */
	var createReducer = function createReducer(_namespace, initState) {
		var otherReducer = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;


		var namespace = _namespace.replace('.', '/');
		...
		var newReducer = function newReducer() {
			...
			//不是该名空间的直接返回
			if (action.type.indexOf(namespace + '/') != 0) return state;

			// 根据条件选择 是否需要 imState 转换
			var imState = void 0;
			if (_immutable.Map.isMap(state)) imState = state;else imState = (0, _immutable.fromJS)(state);
			...
			switch (action.type) {
				...
				case namespace + '/merge':
					if (payload.in) {
						if (payload.deep) imState = imState.mergeDeepIn(payload.arrIndex, payload.diff);else imState = imState.mergeIn(payload.arrIndex, payload.diff);
					} else {
						if (payload.deep) imState = imState.mergeDeep(payload.diff);else imState = imState.merge(payload.diff);
					}
					break;
			...
			if (_immutable.Map.isMap(state)) return imState;else return imState.toJS();
		};

		//合并其他reducer
		var finalReducer = void 0;
		if (otherReducer) {
			console.log('存在合并的 reducer');
			finalReducer = function finalReducer() {
				var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : initState;
				var action = arguments[1];
	 ...
	 */
	var createLoggableActions = function createLoggableActions(namespace, rawActions) {
		var actions = {};
			...
			if (rawActions.hasOwnProperty(actionName)) {
				(function () {
					...
					actions[actionName] = function () {
						...
						rawActions[_actionName].apply(rawActions, arguments);
					};
				})();
			}
		}
	...
	exports.createPartStore = createPartStore;
	exports.createLoggableActions = createLoggableActions;
